...
void Tree::DeleteItem(TreeNode *& SubtreeRoot, DateType item)
...
void Tree::Delete(DateType item)
...
int Tree::BranchHeight(TreeNode *ptrNode){
	...
	//If the node passed in is equel to null, it is an empty tree, just return 0 as height
	if (ptrNode == NULL)
	{
		return 0;
	}

	//recursively call this function to get the height of two branches of the tree

	int left =	BranchHeight(ptrNode->Left);
		
    int right = BranchHeight(ptrNode->Right);
		
	//compare two branches and return the branches with greater height +1
	if (left > right)
		return (left + 1);
...
int Tree::Height(){
	//the  branchHeight function and pass in the root of the tree to get the height of the tree
